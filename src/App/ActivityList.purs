...
sidebarView _ =
  HH.aside
    ...
    [ HH.section [ Prop.classes [ ClassName "pt-[10px]" ] ]
        [ HH.nav [ Prop.classes [ ClassName "text-base" ] ]
            [ HH.ul [ Prop.classes [] ]
                [ HH.li []
                    [ HH.button
                        [ Prop.classes $ isPrimaryCSS <> buttonCSS
...
panelsView state =
  ...
  where
  allPanels' = fromFoldable (flip Tuple [] <$> allPanels)
...
type Slots
  = ( addTodoDialog :: forall query. AddTodoDialog.Slot query Int
...
render state =
  HH.div
    ...
    [ sidebarView state
    , panelsView state
    , if state.showAddTodoModal then
        HH.div_ [ HH.slot _addTodoDialog 0 AddTodoDialog.component 0 HandleAddTodo ]
      else
        HH.div_ []
...
splitTodosByTitle task tasks =
  let
    { init: init, rest: rest } = span (\t -> t.title == task.title) tasks

    maybeTask = head rest

    rest' = fromMaybe [] (tail rest)
  in
    (\t -> { init, task: t, rest: rest' }) <$> maybeTask
